---
id: 114032786
title: Orientações - [DC] Câmbio
version: 6
modified: 2025-04-23T19:23:33.432Z
url: /spaces/OF/pages/114032786/Orienta+es+-+DC+C+mbio
---
...
| Recursos incluídos no compartilhamento | Operações de câmbio ainda em aberto em um período de até 12 meses anterior ao inicio da vigência do consentimento e todos os seus eventos (caso existam), mesmo que contratadas em um período superior a 12 meses; Operações de câmbio canceladas ou liquidadas em um período de até 12 meses anterior ao inicio da vigência do consentimento e todos os seus eventos (caso existam), mesmo que contratadas em um período superior a 12 meses; Operações de câmbio contratadas, canceladas ou liquidadas e todos os seus eventos (caso existam) durante o período de vigência do consentimento, mesmo que contratadas em um período superior a 12 meses; |
| Recursos não incluídos no compartilhamento | Operações de câmbio canceladas ou liquidadas em um período maior que 12 meses anterior ao inicio da vigência do consentimento; Operações de câmbio que pertencem a clientes que estão sob algum tipo de bloqueio conforme políticas internas das instituições; Operações de câmbio anuladas. |
...
*cancelada ou liquidada**Glossário:**
...
### Regras para o retorno da API de produtos de câmbio de acordo com o status do Recurso (PENDING_AUTHORISATION, AVAILABLE, UNAVAILABLE E TEMPORARILY_UNAVAILABLE) e a transição entre status
...
- TEMPORARILY_UNAVAILABLE : indica que o recurso (câmbio) se encontra temporariamente indisponível para consulta nas API de Câmbio do produto em questão. Neste caso a Transmissora deve retornar um erro HTTP Status Code 403 - Forbidden com o conteúdo abaixo:
...
| Cenário | Status do recurso na transmissora | API Recursos (…/resources/v1/resources) | API de listagem de Produto (/exchanges)¹ | APIs de Dados de Produtos (/exchanges/{operationId})² |
| --- | --- | --- | --- | --- |
...
API **Legenda**:1 - Válido para a listagem de todos os produtos de câmbio.2 - Válido para todos os endpoints de produtos de câmbio.3 - Para os cenários de consentimento revogado ou expirado, conforme determinado na especificação de segurança o access token associado ao mesmo é invalidado impossibilitando a consulta às API de Câmbio por parte da Receptora.
