---
id: 1276741090
title: Cenário 11: payroll-credit-portability_api_creditor-rejected-payment-nofound_test-module_v1 - CF
version: 2
modified: 2025-12-01T17:15:29.577Z
url: /spaces/OF/pages/1276741090/Cen+rio+11+payroll-credit-portability_api_creditor-rejected-payment-nofound_test-module_v1+-+CF
---

| Tipo do teste | Teste funcional |
| Descrição | Rejeição de portabilidade pela credora por decurso de prazo do pagamento. |
| Objetivo | Garante que a credora possa rejeitar um pedido de solicitação portabilidade ao ocorrer decurso de prazo do pagamento. |
...
| 10 | Serão feitas a chamadas de endpoint GET /portabilities/{portabilityId} a cada 10 minutos enquanto status estiver em "RECEIVED" ou "PENDING" ou “ACCEPTED_SETTLEMENT_IN_PROGRESS” | Status 200 - Validação de resposta de acordo com a especificação e data.status igual "ACCEPTED_SETTLEMENT_COMPLETED" | Etapa de contraproposta e Etapa de liquidação e Etapa de validação da liquidação |
| 11 | Serão feitas a chamadas de endpoint GET /portabilities/{portabilityId} a cada 10 minutos enquanto status estiver em "ACCEPTED_SETTLEMENT_COMPLETED" | Status 200 - Validação de resposta de acordo com a especificação e data.status igual "REJECTED" e statusReason.reasonType como "DECURSO_DO_PRAZO_PARA_PAGAMENTO" | Etapa de validação da liquidação |
| 12 | Fim do teste | | |
