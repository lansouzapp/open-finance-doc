---
id: 1276741822
title: Cenário 19: payroll-credit-portability_api_expired-consent_test-module_v1 - CF
version: 2
modified: 2025-12-01T17:16:36.751Z
url: /spaces/OF/pages/1276741822/Cen+rio+19+payroll-credit-portability_api_expired-consent_test-module_v1+-+CF
---

| Tipo do teste | Teste de exceção |
| Descrição | Validações de segurança no comportamento das requisições |
| Objetivo | Garante que o sistema impeça que ocorra uma portabilidade com um consentimento expirado |
...
| 8 | Chamada de endpoint GET Loans Contracts/{contractId}/payments | 200 - Validate the response and retrieve contractOutstandingBalance |
| 9 | Rotina de tempo para chamada de endpoint GET consents para 5 mnutos, uma chamada a cada 30 segundos | |
...
| 12 | Chamada de endpoint POST /portabilities com um x-fapi-interaction-id inválido | Status 400 - Validação de mensagem garantindo que o x-fapi-interaction-id inválido foi enviado |
| 13 | Chamada de endpoint POST /portabilities enviando os seguintes valores: data.institution.creditor.companyCnpj = original companyCnpj instalmentPeriodicity = original instalmentPeriodicity proposedContract.totalNumberOfInstalments = original dueInstalments contractAmount = original contractOutstandingBalance valid x-fapi-interaction-id | Status 202 - Validação de resposta de acordo com a especificação e garante que o x-fapi-interaction-id esta correto |
| 14 | Fim do teste | |
