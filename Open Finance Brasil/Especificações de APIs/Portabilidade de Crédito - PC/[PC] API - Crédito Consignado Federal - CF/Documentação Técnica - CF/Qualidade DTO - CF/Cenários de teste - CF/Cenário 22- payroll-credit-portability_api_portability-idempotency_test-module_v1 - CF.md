---
id: 1276742169
title: Cenário 22: payroll-credit-portability_api_portability-idempotency_test-module_v1 - CF
version: 2
modified: 2025-12-01T17:16:55.262Z
url: /spaces/OF/pages/1276742169/Cen+rio+22+payroll-credit-portability_api_portability-idempotency_test-module_v1+-+CF
---

| Tipo do teste | Teste de exceção |
| Descrição | Validações de segurança no comportamento das requisições |
| Objetivo | Garante que o sistema impeça que ocorra uma portabilidade quando houver idempotencia. |
...
| 9 | Chamada do endpoint POST /portabilities enviando os seguintes valores: data.institution.creditor.companyCnpj = original companyCnpj instalmentPeriodicity = original instalmentPeriodicity proposedContract.totalNumberOfInstalments = original dueInstalments contractAmount = original contractOutstandingBalance | |
| 10 | Chamada de endpoint POST /portabilities com o mesmo x-idempotency-key e payload da última requisição | Status 202 - Resposta de sucesso |
| 11 | Chamada de endpoint POST /portabilities com o mesmo x-idempotency-key mas com payload diferentes da última requisição | Status 422 - Validação de resposta esperando código de erro como "ERRO_IDEMPOTENCIA" |
| 12 | Fim do teste | |
