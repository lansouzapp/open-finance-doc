---
id: 1276741327
title: Cenário 14: payroll-credit-portability_api_proposer-rejected-discharge-paymentreversal_test-module_v1 - CF
version: 2
modified: 2025-12-01T17:15:53.160Z
url: /spaces/OF/pages/1276741327/Cen+rio+14+payroll-credit-portability_api_proposer-rejected-discharge-paymentreversal_test-module_v1+-+CF
---

| Tipo do teste | Teste funcional |
| Descrição | Rejeição de portabilidade devido a inconsistências no processo de desaverbação |
| Objetivo | Garante que a proponente consegue rejeitar a portabilidade devido a inconsistência no processo de desaverbação. |
...
| 14 | Serão feitas a chamadas de endpoint GET /portabilities/{portabilityId} a cada 10 minutos enquanto status estiver em “ACCEPTED_SETTLEMENT_COMPLETED” | Status 200 - Validação de resposta de acordo com a especificação e data.status igual "AWAITING_CONTRACT_DISCHARGE" | Etapa de desaverbação de contrato |
| 15 | Chamada de endpoint POST /portabilities/{portabilityId}/payment-reversal | Status 200 - Validação de resposta de acordo com a especificação | Etapa de solicitação de reembolso de liquidação e Etapa de rejeição da solicitação de portabilidade |
| 16 | Serão feitas a chamadas de endpoint GET /portabilities/{portabilityId} a cada 10 minutos enquanto status estiver em "AWAITING_CONTRACT_DISCHARGE" | Status 200 - Validação de resposta de acordo com a especificação e data.status igual "REJECTED" e data.rejection.rejectdBy como "PROPONENTE" e statusReason.reasonType como "RESERVA_DE_MARGEM" |
| 17 | Fim do teste | | |
