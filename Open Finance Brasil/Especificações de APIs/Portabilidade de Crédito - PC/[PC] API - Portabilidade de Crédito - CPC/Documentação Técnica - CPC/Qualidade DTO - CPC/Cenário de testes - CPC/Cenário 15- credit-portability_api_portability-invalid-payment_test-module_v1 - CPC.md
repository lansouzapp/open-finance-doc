---
id: 1329299915
title: Cenário 15: credit-portability_api_portability-invalid-payment_test-module_v1 - CPC
version: 1
modified: 2025-12-01T16:58:14.150Z
url: /spaces/OF/pages/1329299915/Cen+rio+15+credit-portability_api_portability-invalid-payment_test-module_v1+-+CPC
---

| Tipo do teste | Teste funcional |
| Descrição | Criação de solicitação de portabilidade com pagamento inválido. |
| Objetivo | Garantir que há validações quando houver inconsistências no pagamento |
...
| 9 | Chamada de endpoint POST /portabilities utilizando os seguintes valores: data.institution.creditor.companyCnpj = original companyCnpj instalmentPeriodicity = original instalmentPeriodicity proposedContract.totalNumberOfInstalments = original dueInstalments contractAmount = original contractOutstandingBalance | Status 202 - Validação de resposta de acordo com especificação | Etapa de solicitação de portabilidade |
| 10 | Rotina de tempo com a chamada de endpoint GET /portabilities/{portabilityId} por 10 minutos enquanto encontrar status igual a “RECEIVED” ou “PENDING” | Status 200 - Validação de resposta e garantia que o status seja igual "ACCEPTED_SETTLEMENT_IN_PROGRESS" | Etapa de contra proposta e liquidação de contrato |
| 11 | Chamada de endpoint GET /account-data | Status 200 - Validação de resposta de acordo com especificação | Etapa de liquidação de contrato |
...
| 13 | Rotina de tempo com a chamada de endpoint GET /portabilities/{portabilityId} por 10 minutos enquanto encontrar status igual a “ACCEPTED_SETTLEMENT_IN_PROGRESS” | Status 200 - Validação de resposta e garantia que o status seja igual "ACCEPTED_SETTLEMENT_COMPLETED" | Etapa de validação de liquidação de contrato |
| 14 | Rotina de tempo com a chamada de endpoint GET /portabilities/{portabilityId} por 10 minutos enquanto encontrar status igual a ACCEPTED_SETTLEMENT_COMPLETED | Status 200 - Validação de resposta e garantia que o status seja igual "PAYMENT_ISSUE" |
| 15 | Fim do teste | | |
