---
id: 1183023105
title: Documento de Requisitos do Produto - [PC] Portabilidade de Crédito - CPC
version: 6
modified: 2025-11-25T13:54:56.145Z
url: /spaces/OF/pages/1183023105/Documento+de+Requisitos+do+Produto+-+PC+Portabilidade+de+Cr+dito+-+CPC
---
...
As seguintes adequações serão implementadas na API de Empréstimos para viabilizar a solicitação de portabilidade de crédito para o produto CPC:**Campos existentes:**
...
O cliente poderá desistir do pedido de portabilidade de crédito a qualquer momento antes de ocorrer a liquidação da operação, tanto no canal digital da instituição proponente quanto no canal digital da instituição credora original.**Ponto importante:**O prazo final para desistência do pedido será às 9h do terceiro dia útil após a solicitação ou até a atualização da máquina de estados pela instituição credora original.**Procedimentos:**
...
- Dados bancários para a transferência de recursos via TED.
**Procedimento:**
...
A instituição credora original terá até 2 dias úteis para confirmar o recebimento dos recursos referentes à liquidação da operação, atestando a efetivação da portabilidade. O prazo será contado a partir do próximo dia útil, independentemente do horário em que o pagamento seja realizado.Na contagem dos dias, devem ser considerados dias úteis somente os dias de segunda a sexta-feira, exceto os feriados nacionais.**Comunicação:**
- A confirmação do recebimento dos recursos e a efetivação do contrato devem ser comunicadas através do serviço de notificação, baseado em eventos.
...
**Ponto importante:**Nos casos em que for necessário realizar o estorno, este deve ocorrer de forma integral e exclusivamente via STR0010 (motivo 85) para o fluxo de portabilidade de crédito no OFB.
...
Esta seção apresenta os indicadores do produto que devem ser monitorados pelas ferramentas da estrutura.
- Categoria: Taxa de adoção Quantidade/Percentual de instituições que implementaram o serviço de PC via OFB Número de solicitações de portabilidade realizadas, através dos filtros de Proponente e Credora Número de solicitações de portabilidade realizadas por tipo de contrato, através dos filtros de Proponente e Credora Incidência de múltiplas solicitações para o mesmo contrato Proporção de propostas aceitas versus propostas enviadas/efetivadas Frequência de recusas por modelo de contrato Número de pedidos concluídos com sucesso, através dos filtros de Proponente e Credora Percentual de crescimento da adesão das instituições financeiras que utilizam a API de Portabilidade Percentual de crescimento da utilização do serviço de portabilidade de crédito
- Categoria: Indicadores técnicos Percentual de tempo em que a API está operacional e disponível Número de respostas de erro (HTTP 408 e 5xx) em relação ao total de requisições Tempo médio de resposta para requisições à API Tempo que uma requisição leva para ir do cliente para o servidor e vice-versa Número de tentativas de acesso não autorizado detectadas Capacidade da API de lidar com picos de carga sem degradação de desempenho Número de chamadas que funcionam sem problemas em diferentes versões da API Percentual de chamadas bem-sucedidas (2XX e 422) em relação às tentativas Feedback dos desenvolvedores sobre a clareza e utilidade da documentação Tempo médio que um desenvolvedor leva para implementar a API pela primeira vez Qualidade dos dados trafegados entre as APIs
- Categoria: Tempo médio de solicitação e prazos Tempo médio para conclusão do pedido de portabilidade por situação do pedido de portabilidade, considerando a máquina de estados Número de pedidos de portabilidade não finalizados dentro do prazo regulatório
- Categoria: Variações das operações retidas Número de solicitações de portabilidade retidas pela credora Média da taxa de juros das operações retidas após a solicitação de portabilidade Percentual de solicitações de portabilidade retidas pela credora
- Categoria: Taxa de sucesso Portabilidade efetivadas + retenções

# 6. Perguntas e respostas
Esta seção reúne as principais dúvidas sobre o fluxo de portabilidade de crédito via OFB para o produto Crédito Pessoal Clean (empréstimo sem consignação). As perguntas e respostas foram organizadas por tema para facilitar a consulta.
...
8. Com a disponibilização do fluxo de PC no OFB, ainda será possível realizar a solicitação de PC via registradora (Núclea)? Sim, será possível, porém será necessário que a instituição credora implemente políticas para impedir simultaneidade.
...
# 7. Glossário
● **API:** Interface de Programação de Aplicações.● **BCB:** Banco Central do Brasil.● **CCB:** Cédula de Crédito Bancário.● **CET:**Custo Efetivo Total.● **CPC:** Crédito Pessoal Clean.● **GT:** Grupo de Trabalho.● **IF:** Instituição Financeira.● **Núclea:**Plataforma de registro de operações de crédito.● **OFB:** Open Finance Brasil.● **PC:** Portabilidade de Crédito.●**PRD:** Documento de Requisitos de Produto.● **RCO:** Ressarcimento de Custo de Originação.● **STR:** Sistema de Transferência de Reservas.● **TED:** Transferência Eletrônica Disponível.● **UUID:** Universally Unique Identifier.● **IPCA:** Índice Nacional de Preços ao Consumidor Amplo● **IGP-M:** Índice Geral de Preços – Mercado
# 8. Apêndice
